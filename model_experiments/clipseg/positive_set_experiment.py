"""Positive-set experiment for CLIPSeg (paper's E3): for every eligible benchmark
...
--alpha should be the value derived by alpha_value_experiment.py's COCO sweep for CLIPSeg.
...
def predict_masks_for_tags(model, image, tag_to_embedding, threshold=0.5):
    """CLIPSeg's embeddings_dict keys are arbitrary labels, so every tag for one image
...
if __name__ == "__main__":
    ...
    out_path = os.path.join(args.out_dir, "positive_set_experiment_clipseg.csv")
    summary_path = os.path.join(args.out_dir, "positive_set_experiment_clipseg_summary.csv")
