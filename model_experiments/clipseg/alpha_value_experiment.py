...
weighted-centroid blending toward the WordNet synset centroid) -- CLIPSeg.

This is the CLIPSeg copy of the per-model alpha-derivation experiment: run this script
once per model (see the sibling groupvit/ and sclip/ folders for the others) to find that
...
The blending math, WordNet disambiguation, and CLIPSeg inference are self-contained here
(clipseg.py, base.py, and this file), not imported from the wider repo.
...
# =============================================================================
# Model (CLIPSeg via the local wrapper). get_text_embedding_cached / run_segmentation_batch
# are the two calls the rest of this script needs; swap this section for the sibling
# groupvit/sclip folders' equivalent wrapper to retarget the same sweep at another model.
# =============================================================================
...
if __name__ == "__main__":
    ...
    detail_out = os.path.join(args.out_dir, "alpha_value_experiment_clipseg_detail.csv")
    summary_out = os.path.join(args.out_dir, "alpha_value_experiment_clipseg_summary.csv")
