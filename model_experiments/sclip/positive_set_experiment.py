"""Positive-set experiment for SCLIP (paper's E3): for every eligible benchmark
...
--alpha should be the value derived by alpha_value_experiment.py's COCO sweep for SCLIP.

SCLIP performance note: unlike clipseg/groupvit, its joint-softmax interface only accepts
one embedding per class per call (see sclip.py's module docstring), so
