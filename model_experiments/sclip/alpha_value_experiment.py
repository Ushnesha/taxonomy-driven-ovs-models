...
weighted-centroid blending toward the WordNet synset centroid) -- SCLIP.

This is the SCLIP copy of the per-model alpha-derivation experiment: run this script once
per model (see the sibling clipseg/ and groupvit/ folders for the others) to find that
...
nothing else from the wider repo checked out -- see requirements.txt / setup_env.sh in
this same folder (SCLIP needs mmengine/mmcv/mmsegmentation, heavier than clipseg/groupvit's
deps). The blending math, WordNet disambiguation, and the vendored SCLIP repo are
self-contained here (sclip.py, base.py, SCLIP/, and this file), not imported from the
wider repo.

SCLIP performance note: unlike clipseg/groupvit, SCLIP's joint-softmax interface only
accepts one embedding per class per call (see sclip.py's module docstring), so
run_segmentation_batch below issues one predict_with_embeddings call per (variant,
weighted, alpha) tag instead of one batched call -- expect this sweep to run substantially
slower than the other two models at full scale.
...
# =============================================================================
# Model (SCLIP via the local wrapper). get_text_embedding_cached / run_segmentation_batch
# are the two calls the rest of this script needs; swap this section for the sibling
# clipseg/groupvit folders' equivalent wrapper to retarget the same sweep at another model.
# =============================================================================
...
def get_model():
    ...
    if _model is None:
        ...
        # No name_path override -- SCLIP's own default (configs/cls_coco_object.txt,
        # inside the vendored SCLIP/ repo) already covers COCO's 80 classes, which is
        # exactly this script's scope.
        _model = SClipModel()
    return _model
...
def run_segmentation_batch(image, cond_embeddings, cat_name, threshold=0.5):
    ...
    So this loops one call per embedding, all keyed by `cat_name` (the actual class every
    ...
    try:
        ...
        for cat_name in categories:
            ...
            for img_ref in img_refs:
                ...
                masks = run_segmentation_batch(image, embeddings, cat_name)
...
if __name__ == "__main__":
    ...
    detail_out = os.path.join(args.out_dir, "alpha_value_experiment_sclip_detail.csv")
    summary_out = os.path.join(args.out_dir, "alpha_value_experiment_sclip_summary.csv")
