...
# --- Cell 17 ---
print("With Hypo categories:", hypo_cats)
fig, gt_overlay, pred_overlay, gt_masks, pred_masks = visualize_segmentation_with_labels(image, annotations, hypo_cats, coco, model, processor)
plt.show()
...
# --- Cell 25 ---
cats = ['bottle', 'person', 'dining table', 'carrot', 'bowl', 'spoon', 'broccoli', 'knife', 'oven', 'cup', 'sink']
orig_cats, syn_cats, hyper_cats, hypo_cats = {}, {}, {}, {}
for cat in cats:
    orig_cats[cat] = cat
    synset = wordnet.synsets(cat)[0] if wordnet.synsets(cat) else None
    ...
    if hyponym is None:
        hyponym_bblNet = _babelnet_hyponym(cat)
    hypo_cats[cat] = hyponym if hyponym is not None else hyponym_bblNet
    # print(f"Category: {cat}, Hyponym (WordNet): {hyponym}, Hyponym (BabelNet): {hyponym_bblNet}")

print("\nFinal category mappings:")
for cat in cats:
    print(f"Category: {cat}, Synonym: {syn_cats[cat]}, Hypernym: {hyper_cats[cat]}, Hyponym: {hypo_cats[cat]}")

# --- Cell 26 ---
synonym = _conceptnet_synonym('dining table')
print(synonym)
