...
# --- Cell 4 ---
def get_openseg_model(device=None):
    """
    Load OpenSEG-style model using CLIP backbone with mask proposals.
    
    OpenSEG approach:
    1. Generate class-agnostic mask proposals (using simple methods)
    2. Use CLIP to score/classify each proposal with text prompts
    3. Combine scores to create final segmentation
    
    For simplicity, we use CLIP features directly with a region proposal approach.
    """
    if device is None:
        device = get_device()
...
def visualize_segmentation_with_labels(image, annotations, text_cats, coco, model, processor, threshold = 0.5, desc = False):
    ...
    for idx, (p_base, p_var) in enumerate(text_cats.items()):
        ...
        # print(f"Original image type: {type(image)}, size: {image.size if hasattr(image, 'size') else 'N/A'}")
        # image_resized = image.resize((224, 224))
        # print(f"Resized image type: {type(image_resized)}, size: {image_resized.size}")
...
# fig, gt_overlay, pred_overlay, gt_masks, pred_masks = visualize_segmentation_with_labels(image, annotations, hyper_cats, coco, model, processor)
# plt.show()
...
for img_id in tqdm(img_ids, desc="Processing images"):
    try:
        ...
        for cat_type, cat_dict in category_types.items():
            ...
            miou, ious = calculate_miou(gt_masks, pred_masks)
            # results[cat_type]['img_id'].append(img_id)
            # results[cat_type]['mIoU'].append(miou)
            # results[cat_type]['categories'].append(list(cat_dict.values()))
            # results[cat_type]['IoUs'].append(ious)
...
for v in VARIANTS:
    smoothed = rolling_mean(variant_vals[v], w=15)
    ax1.plot(x, smoothed, marker='o', ms=4, lw=1.6,
             label=f'μ {v}', color=VARIANT_COLORS[v], alpha=0.85)

smoothed_mu = rolling_mean(mu_vals, w=15)
ax1.plot(x, smoothed_mu, marker='D', ms=4, lw=2, ls='--',
