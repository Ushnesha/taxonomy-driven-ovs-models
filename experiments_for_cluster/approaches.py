...
SHiNe / WaffleCLIP / LLM-descriptor are ported near-verbatim from
../helper_functions.py (itself ported from experiments_for_cluster/
{shine,waffleclip,llm_description}_experiment.py). This module always has a
real `model` to embed against (no CLIPSeg-only bare-cache fallback), so the
model=None branches those functions carried are dropped here.
...
from expanded_benchmark_helpers import (  # noqa: E402
    to_display_form,
    find_best_synset,
